...
async function assertCompanyOwnership(companyId: string) {
  ...
  const { data: company, error } = await supabase.from("companies").select("id").eq("id", companyId).single();
...
export async function extractStatement(companyId: string, tipo: "cartao" | "banco", formData: FormData): Promise<ExtractResult> {
  try {
    ...
    const { data: customCats } = await supabase.from("categories").select("name").eq("company_id", companyId);
    ...
    if (result.rows.length === 0) return { error: "Não consegui identificar transações nesse extrato. Tente colar o texto puro." };
...
export async function confirmImport(companyId: string, tipo: "cartao" | "banco", account: string, rows: { date: string; description: string; amount: number; category: string }[]): Promise<ConfirmImportResult> {
  try {
    if (!rows || rows.length === 0) return { error: "Nenhuma transação selecionada." };
    const supabase = await assertCompanyOwnership(companyId);
    const { error } = await supabase.from("transactions").insert(rows.map((r) => ({ company_id: companyId, date: r.date, description: String(r.description || "").slice(0, 60), amount: r.amount, category: r.category, account: account || "Conta sem nome", tipo })));
...
export async function updateTransactionCategory(companyId: string, transactionId: string, category: string) {
  const supabase = await assertCompanyOwnership(companyId);
  const { error } = await supabase.from("transactions").update({ category }).eq("id", transactionId).eq("company_id", companyId);
...
export async function deleteTransactionAction(companyId: string, transactionId: string) {
  const supabase = await assertCompanyOwnership(companyId);
  const { error } = await supabase.from("transactions").delete().eq("id", transactionId).eq("company_id", companyId);
  if (error) throw new Error(error.message);
  revalidateCompany(companyId);
}

export type ManualState = { error?: string } | null;
export async function addManualTransaction(companyId: string, tipo: "cartao" | "banco", _prevState: ManualState, formData: FormData): Promise<ManualState> {
  ...
  if (!date || !description || Number.isNaN(amount)) return { error: "Preencha data, descrição e valor." };
  const supabase = await assertCompanyOwnership(companyId);
  const { error } = await supabase.from("transactions").insert({ company_id: companyId, date, description: description.slice(0, 60), amount, category, account, tipo });
...
export async function bulkDeleteTransactions(
  ...
  try {
    const supabase = await assertCompanyOwnership(companyId);
    let query = supabase.from("transactions").delete({ count: "exact" }).eq("company_id", companyId).eq("tipo", tipo);
    if (mode === "month") {
      if (!month || !/^\d{4}-\d{2}$/.test(month)) return { error: "Selecione um mês válido." };
      const [y, m] = month.split("-").map(Number);
      const nextDate = new Date(y, m, 1);
      query = query.gte("date", `${month}-01`).lt("date", `${nextDate.getFullYear()}-${String(nextDate.getMonth() + 1).padStart(2, "0")}-01`);
    } else if (mode === "last7days") {
      const d = new Date(); d.setDate(d.getDate() - 7);
      query = query.gte("date", d.toISOString().slice(0, 10));
    }
    if (account) query = query.eq("account", account);
    if (direction === "entrada") query = (query as typeof query).gt("amount", 0);
    if (direction === "saida") query = (query as typeof query).lt("amount", 0);
...
export async function bulkDeleteByIds(companyId: string, ids: string[]): Promise<BulkDeleteState> {
  try {
    if (!ids.length) return { error: "Nenhuma transação selecionada." };
    const supabase = await assertCompanyOwnership(companyId);
    const { error, count } = await supabase.from("transactions").delete({ count: "exact" }).eq("company_id", companyId).in("id", ids);
    if (error) return { error: error.message };
    revalidateCompany(companyId);
    return { success: `${count ?? ids.length} transação(ões) apagada(s).` };
  } catch (e) {
    return { error: e instanceof Error ? e.message : "Erro ao apagar." };
  }
}

export async function bulkUpdateCategory(companyId: string, ids: string[], category: string): Promise<{ error?: string }> {
  try {
    if (!ids.length) return { error: "Nenhuma transação selecionada." };
    const supabase = await assertCompanyOwnership(companyId);
    const { error } = await supabase.from("transactions").update({ category }).eq("company_id", companyId).in("id", ids);
    if (error) return { error: error.message };
    revalidateCompany(companyId);
    return {};
  } catch (e) {
    return { error: e instanceof Error ? e.message : "Erro ao atualizar categoria." };
  }
}
